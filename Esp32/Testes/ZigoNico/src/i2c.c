...
void i2c_write(uint8_t *tx_data,float *angular_speed_left,float *angular_speed_right){
    ...
    i2c_slave_write_buffer(I2C_PORT,tx_data,MENSAGE_SIZE,TIMEOUT_MS_WRITE / portTICK_RATE_MS);
...
void i2c_read(uint8_t *rx_data,float *angular_speed_left,float *angular_speed_right){
    ...
    if(i2c_slave_read_buffer(I2C_PORT,rx_data,MENSAGE_SIZE,TIMEOUT_MS_READ / portTICK_RATE_MS) > 0){
